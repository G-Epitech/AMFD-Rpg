...
static bool fill_array(cjson_array_t *cjson, int *array, size_t size)
{
    bool valid = true;
...
cjson_array_t *cjson_array_from_int_array(int *array, size_t size)
{
    ...
    if (!cjson) {
        internal_cjson_error("cjson array from int array failed (malloc)",
        NULL);
        return NULL;
    }
    if (!array)
    ...
    if (!fill_array(cjson, array, size)) {
        internal_cjson_error("cjson array filling from int failed (malloc)",
        NULL);
