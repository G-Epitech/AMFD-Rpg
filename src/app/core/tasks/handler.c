...
int core_tasks_handler(app_t *app, renderer_t *renderer)
{
    if (app->state == ST_TASK_BASH) {
        if (app_task_bash_core(app, renderer) == 84)
            return 84;
    }
    if (app->state == ST_TASK_BRUTEFORCE) {
        if (app_task_brute_core(app, renderer) == 84)
            return 84;
    }
    if (app->state == ST_TASK_FLIPPER) {
        if (app_task_flipper_core(app, renderer) == 84)
            return 84;
    }
    if (app->state == ST_TASK_CAMERA) {
        if (app_task_camera_core(app, renderer) == 84)
            return 84;
    }
    return 0;
}
